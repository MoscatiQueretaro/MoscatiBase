package org.iconotecnologies.damner.web.rest.agora.sample;

import java.util.Map;
import java.util.TreeMap;
import org.apache.commons.codec.binary.Hex;
import org.iconotecnologies.damner.web.rest.agora.media.DynamicKey5;
...
public class Verifier5 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (key5.content.serviceType == DynamicKey5.MEDIA_CHANNEL_SERVICE) {
            System.out.println(
                "Uid = 0 \t\t " +
                DynamicKey5.generateMediaChannelKey(
                    appID,
                    appCertificate,
                    channelName,
                    key5.content.unixTs,
            ...
            System.out.println(
                "Uid = 0 \t\t " +
                DynamicKey5.generateRecordingKey(
                    appID,
                    appCertificate,
                    channelName,
                    key5.content.unixTs,
            ...
            if (permission != DynamicKey5.noUpload && permission != DynamicKey5.audioVideoUpload) {
                System.out.println(
                    "Unknown in channel upload permission " + permission + " in extra [" + toString(key5.content.extra) + "]"
                );
                return;
            }
            System.out.println(
                "Uid = 0 \t\t " +
                DynamicKey5.generateInChannelPermissionKey(
                    appID,
                    appCertificate,
                    channelName,
                    key5.content.unixTs,
        ...
        String signature = DynamicKey5.generateSignature(
            appCertificate,
